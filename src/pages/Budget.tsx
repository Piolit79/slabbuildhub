...
export default function BudgetPage() {
  ...
  const grandOptional = filtered.reduce((s, b) => s + b.optional, 0);
  ...
  const handleAdd = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setItems(prev => [...prev, {
      ...
      optional: parseFloat(fd.get('optional') as string) || 0, subcontractor: fd.get('subcontractor') as string,
      notes: fd.get('notes') as string, status: fd.get('status') as BudgetItem['status'],
    }]);
    setOpen(false);
  };
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-xs">Line-item budget by category</p>
        </div>
          ...
          <DialogContent>
            ...
            <form onSubmit={handleAdd} className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <div className="space-y-1"><Label className="text-xs">Optional</Label><Input name="optional" type="number" step="0.01" defaultValue="0" className="h-8 text-xs" /></div>
              </div>
              <div className="space-y-1"><Label className="text-xs">Subcontractor</Label><Input name="subcontractor" className="h-8 text-xs" /></div>
              <div className="space-y-1"><Label className="text-xs">Notes</Label><Input name="notes" className="h-8 text-xs" /></div>
      ...
      {categories.map(cat => {
        ...
        const sortedCat = [...catItems].sort((a: any, b: any) => {
          const av = a[sortKey], bv = b[sortKey];
          if (typeof av === 'number') return sortDir === 'asc' ? av - bv : bv - av;
          return sortDir === 'asc' ? String(av ?? '').localeCompare(String(bv ?? '')) : String(bv ?? '').localeCompare(String(av ?? ''));
        });
        const catL = catItems.reduce((s, b) => s + b.labor, 0);
        const catM = catItems.reduce((s, b) => s + b.material, 0);
        const catO = catItems.reduce((s, b) => s + b.optional, 0);

        return (
          <Card key={cat}>
            <CardHeader className="pb-0 pt-2 px-3">
              <CardTitle className="text-sm font-bold bg-secondary/60 -mx-3 px-3 py-1">{cat}</CardTitle>
            </CardHeader>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right">{sh('Optional', 'optional', 'justify-end')}</TableHead>
                    ...
                    <TableRow key={b.id}>
                      {editId === b.id ? (
                        <>
                          ...
                          <TableCell><Input value={editData.optional || 0} onChange={e => setEditData(d => ({ ...d, optional: parseFloat(e.target.value) || 0 }))} type="number" className="h-6 text-xs w-20 px-1 text-right" /></TableCell>
                          ...
                          <TableCell><Input value={editData.notes || ''} onChange={e => setEditData(d => ({ ...d, notes: e.target.value }))} className="h-6 text-xs px-1 w-24" /></TableCell>
                          <TableCell>
                        ...
                        <>
                          ...
                          <TableCell className="text-right tabular-nums">{fmt(b.optional)}</TableCell>
                          ...
                          <TableCell><button onClick={() => startEdit(b)} className="text-muted-foreground hover:text-foreground"><Pencil size={12} /></button></TableCell>
                        </>
                      )}
                    </TableRow>
                  ))}
                ...
                <TableFooter>
                  <TableRow>
                    ...
                    <TableCell className="text-right font-semibold tabular-nums">{fmtN(catL)}</TableCell>
                    <TableCell className="text-right font-semibold tabular-nums">{fmtN(catM)}</TableCell>
                    <TableCell className="text-right font-semibold tabular-nums">{catO ? fmtN(catO) : '—'}</TableCell>
                    <TableCell colSpan={4} />
                  </TableRow>
                </TableFooter>
              </Table>
