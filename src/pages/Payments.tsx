...
type VendorGroup = { name: string; count: number; total: number; lastDate: string; items: Payment[] };
...
export default function PaymentsPage({ readOnly }: { readOnly?: boolean }) {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (data) setPayments(data as Payment[]);
      const parts = [];
      if (result.imported > 0) parts.push(`${result.imported} check${result.imported !== 1 ? 's' : ''} imported`);
      if (result.importedCC > 0) parts.push(`${result.importedCC} CC transaction${result.importedCC !== 1 ? 's' : ''} imported`);
      if (result.importedFL > 0) parts.push(`${result.importedFL} field labor payment${result.importedFL !== 1 ? 's' : ''} imported`);
  ...
  const renderEditRow = (p: Payment) => isMobile ? (
    <>
  ...
  const renderDisplayRow = (p: Payment) => (
    <>
                    ...
                    {t.value === 'materials' ? (
                      <>
                        {materialGroups.map((group, gIdx) => (
                          <React.Fragment key={`grp-${group.name}`}>
                            {/* Group summary row */}
                            <TableRow
                              className={group.count > 1 ? 'cursor-pointer hover:bg-muted/30' : undefined}
                              style={gIdx % 2 === 0 ? { backgroundColor: 'rgba(195, 126, 135, 0.12)' } : undefined}
                              onClick={group.count > 1 ? () => toggleVendor(group.name) : undefined}
                              onKeyDown={editId === group.items[0].id && group.count === 1 ? (e: React.KeyboardEvent) => { if (e.key === 'Enter') { e.preventDefault(); saveEdit(); } else if (e.key === 'Escape') cancelEdit(); } : undefined}
                            >
                              {editId === group.items[0].id && group.count === 1 ? renderEditRow(group.items[0]) : (
                                <>
                                  <TableCell className="tabular-nums text-[11px] md:text-sm pr-6 text-muted-foreground">
                                    {format(new Date(group.lastDate), 'MM.dd.yy')}
                                  </TableCell>
                                  <TableCell className="text-[11px] md:text-sm pl-6">
                                  ...
                                  {!isMobile && <TableCell className="pl-6 text-[11px] md:text-sm text-muted-foreground">{group.items[group.items.length - 1].form}</TableCell>}
                                  ...
                                  {!readOnly && (
                                    <TableCell>
                                      {group.count > 1 ? (
                                        expandedVendors.has(group.name)
                                          ? <ChevronUp size={12} className="text-muted-foreground" />
                                          : <ChevronRight size={12} className="text-muted-foreground" />
                                      ) : (
                                        undoInfo?.id === group.items[0].id ? (
                                          <button onClick={handleUndo} className="inline-flex items-center gap-1 text-[10px] font-medium text-primary hover:underline whitespace-nowrap"><Undo2 size={11} /> Undo</button>
                                        ) : confirmDeleteId === group.items[0].id ? (
                                          <div className="flex items-center gap-1 whitespace-nowrap" onClick={e => e.stopPropagation()}>
                                            <span className="text-[10px] text-muted-foreground">Delete?</span>
                                            <button onClick={() => deletePayment(group.items[0].id)} className="text-destructive hover:opacity-70"><Check size={13} /></button>
                                            <button onClick={() => setConfirmDeleteId(null)} className="text-muted-foreground hover:text-foreground"><X size={13} /></button>
                                          </div>
                                        ) : (
                                          <div className="flex gap-1.5" onClick={e => e.stopPropagation()}>
                                            <button onClick={() => startEdit(group.items[0])} className="text-muted-foreground hover:text-foreground"><Pencil size={12} /></button>
                                            <button onClick={() => setConfirmDeleteId(group.items[0].id)} className="text-muted-foreground/40 hover:text-destructive"><Trash2 size={12} /></button>
                                          </div>
                                        )
                                      )}
                                    </TableCell>
                            ...
                            {group.count > 1 && expandedVendors.has(group.name) && group.items.map((p) => (
                              <TableRow key={p.id} style={{ backgroundColor: 'rgba(195, 126, 135, 0.06)' }}
                                onKeyDown={editId === p.id ? (e: React.KeyboardEvent) => { if (e.key === 'Enter') { e.preventDefault(); saveEdit(); } else if (e.key === 'Escape') cancelEdit(); } : undefined}
                              >
                                {editId === p.id ? renderEditRow(p) : renderDisplayRow(p)}
                              </TableRow>
                            ))}
                          </React.Fragment>
                        ))}
